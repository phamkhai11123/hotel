                ...
                <div class="col-md-4 col-sm-4">
                    <div class="panel panel-primary">
                        ...
                        <div class="form-group">
                            ...
                            $ho = isset($nameParts[0]) ? $nameParts[0] : '';
                            ?>
                        ...
                        <div class="form-group">
                            <label>Tên</label>
                            <?php
                            // Lấy phần còn lại sau khi loại bỏ phần họ
                            $ten = implode(' ', array_slice($nameParts, 1));
                            ?>
						...
						<?php
							...
							else
							{
									...
									else
									{
										...
										{
											echo "<script type='text/javascript'> alert('Lỗi khi thêm user vào db')</script>";
        ...
        if (isset($_POST['submit'])) {
            ...
            if ($code1 != $code) {
                ...
                if (!empty($data) && $data[0] > 1) {
                    ...
                    $new = "Chưa xác nhận";
                    $nodays = abs(strtotime($_POST['cout']) - strtotime($_POST['cin'])) / (60 * 60 * 24);
                    $newUser = "INSERT INTO `roombook`(`Title`, `FName`, `LName`, `Email`, `Country`, `Phone`, `TRoom`, `Bed`, `NRoom`, `Meal`, `cin`, `cout`,`stat`,`nodays`) VALUES ('$_POST[title]','$_POST[fname]','$_POST[lname]','$email','$_POST[country]','$_POST[phone]','$_POST[troom]','$_POST[bed]','$_POST[nroom]','$_POST[meal]','$_POST[cin]','$_POST[cout]','$new','$nodays')";
                    if (mysqli_query($con, $newUser)) {
                        echo "<script type='text/javascript'> alert('Đơn đặt phòng của bạn đã được gửi')</script>";
                    } else {
                        echo "<script type='text/javascript'> alert('Lỗi khi thêm đơn vào db')</script>";
                    }
                }
            }
        }
