...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['id'])) {
        ...
        $deleteSql = "DELETE FROM `room` WHERE id = '$roomId'";
        if (mysqli_query($con, $deleteSql)) {
            echo "<script type='text/javascript'>      
            alert('Phòng đã được xóa');
            </script>";
        } else {
            echo "<script type='text/javascript'> alert('Đã xảy ra lỗi khi xóa ')</script>";
        }
    }
}
?> 
                ...
                <div class='panel panel-primary text-center no-boder '>
                    <div class='panel-body $buttonColorClass'>
						...
						<div class='panel-footer '>
							" . $roomType . "
							<button class='btn btn-sm text-body t' onclick='showDeleteRoomModal($roomId)'>Delete Room</button>
						</div>
                </div>
            </div>";
    }
